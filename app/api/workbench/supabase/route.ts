// Source: bolt.diy/app/routes/api.supabase.ts
// Ported: Remix → Next.js route handler

import { NextRequest, NextResponse } from 'next/server';
export const dynamic = 'force-dynamic';

import type { SupabaseProject } from '@/lib/workbench/types/supabase';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!projectsResponse.ok) {
      ...
      return NextResponse.json({ error: 'Failed to fetch projects' }, { status: 401 });
    }
