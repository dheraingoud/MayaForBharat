// Source: bolt.diy/app/routes/api.supabase-user.ts
...
async function supabaseUserLoader({ request }: { request: Request }) {
  try {
    ...
    if (!supabaseToken) {
      return NextResponse.json({ error: 'Supabase token not found' }, { status: 401 });
    }
...
export const GET = withSecurity(supabaseUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});
...
export const POST = withSecurity(supabaseUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});
