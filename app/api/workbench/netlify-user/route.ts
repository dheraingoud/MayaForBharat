// Source: bolt.diy/app/routes/api.netlify-user.ts
...
async function netlifyUserLoader({ request }: { request: Request }) {
  try {
    ...
    if (!netlifyToken) {
      return NextResponse.json({ error: 'Netlify token not found' }, { status: 401 });
    }
    ...
    if (!response.ok) {
      if (response.status === 401) {
        return NextResponse.json({ error: 'Invalid Netlify token' }, { status: 401 });
      }
...
export const GET = withSecurity(netlifyUserLoader, {
  rateLimit: true,
  allowedMethods: ['GET'],
});
...
export const POST = withSecurity(netlifyUserAction, {
  rateLimit: true,
  allowedMethods: ['POST'],
});
