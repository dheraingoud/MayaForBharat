...
// Writes directly to app/icons.css with UTF-8 encoding (no BOM)

const fs = require('fs');
const path = require('path');

const iconSetPath = path.resolve(__dirname, '../node_modules/@iconify-json/ph/icons.json');
...
const usedIcons = [
  ...
  'book','book-open','brackets-curly','brain','browser','bug','buildings',
  'camera-plus','caret-down','caret-down-bold','caret-left',
...
let css = `/* AUTO-GENERATED Phosphor Icons for bolt.diy workbench — DO NOT EDIT */
...
for (const name of usedIcons) {
  ...
  if (!icon) {
    console.error(`WARNING: icon "${name}" not found`);
    missing++;
    continue;
  }
  found++;
  ...
  css += `  -webkit-mask-image: ${dataUri};\n`;
  css += `}\n`;
}

const outPath = path.resolve(__dirname, '../app/icons.css');
fs.writeFileSync(outPath, css, 'utf-8');
console.log(`Generated ${found} icons (${missing} missing) -> ${outPath}`);
