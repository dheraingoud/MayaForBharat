...
import type { GitHubUserResponse, GitHubConnection } from '@/lib/workbench/types/GitHub';
import { useGitHubAPI } from './useGitHubAPI';
import { githubConnection, isConnecting, updateGitHubConnection } from '@/lib/workbench/stores/github';
...
export function useGitHubConnection(): UseGitHubConnectionReturn {
  ...
  const testConnection = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (isServerSide) {
        const response = await fetch('/api/workbench/github-user');
        return response.ok;
      }
