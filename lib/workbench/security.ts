// Next.js compatible security utilities (ported from Remix)
// Next.js 16 route handler signature: (request: Request, ctx?: { params?: ... }) => Promise<Response>
// We accept a wider RouteArgs internally so loader-style handlers ({ request }: { request: Request })
// remain assignable, then normalize to a single positional signature at the wrapper boundary.
type RouteArgs = { request: Request; context?: { params?: Record<string, string> | Promise<Record<string, string>> } };
...
export function withSecurity<T extends (args: RouteArgs) => Promise<Response>>(
  ...
  // Next.js 16 route handlers receive (request, context) as positional args
  // params is now Promise<Record<string, string>> in Next.js 16
  return async (request: Request, context?: { params?: Record<string, string> | Promise<Record<string, string>> }): Promise<Response> => {
    const resolvedParams = context?.params ? (context.params instanceof Promise ? await context.params : context.params) : undefined;
    const args: RouteArgs = { request, context: { params: resolvedParams as Record<string, string> | undefined } };
