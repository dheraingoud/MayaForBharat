...
import type { Template } from '@/lib/workbench/types/template';
...
export const selectStarterTemplate = async (options: { message: string; model: string; provider: ProviderInfo }) => {
  ...
  const response = await fetch('/api/workbench/llmcall', {
    method: 'POST',
...
const getGitHubRepoContent = async (repoName: string): Promise<{ name: string; path: string; content: string }[]> => {
  try {
    ...
    const response = await fetch(`/api/workbench/github-template?repo=${encodeURIComponent(repoName)}`);
