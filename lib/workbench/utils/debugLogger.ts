...
const getLogStore = () => {
  if (!logStore && typeof window !== 'undefined') {
    try {
      ...
      import('@/lib/workbench/stores/logs')
        .then(({ logStore: store }) => {
          logStore = store;
        })
        .catch(() => {
...
class DebugLogger {
  ...
  private async _getLogStoreLogs(): Promise<LogEntry[]> {
    try {
      ...
      if (!store) {
        ...
        try {
          const { logStore: storeModule } = await import('@/lib/workbench/stores/logs');
  ...
  private async _getGitInfo(): Promise<AppInfo['gitInfo']> {
    try {
      ...
      const response = await fetch('/api/workbench/system-git-info');
