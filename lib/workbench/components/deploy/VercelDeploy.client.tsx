...
import { vercelConnection } from '@/lib/workbench/stores/vercel';
...
export function useVercelDeploy() {
  ...
  const handleVercelDeploy = async () => {
    ...
    const deployToken = vercelConn.token || process.env?.DEPLOY_TOKEN || '';

    if (!deployToken) {
      toast.error('Deployment token not configured. Please contact support.');
      return false;
    }
    ...
    try {
      ...
      if (!buildOutput || buildOutput.exitCode !== 0) {
        const errorSnippet = buildOutput?.output
          ? buildOutput.output.slice(-500).trim()
        ...
        // Surface actual build error to user
        console.error('[Vercel Deploy] Build failed with output:', errorSnippet);
      ...
      const response = await fetch('/api/workbench/vercel-deploy', {
        method: 'POST',
        headers: {
      ...
      (async () => {
        try {
          const res = await fetch('/api/verify', {
            ...
            body: JSON.stringify({ url: data.deploy.url, source: 'vercel' }),
          });
          if (!res.ok) return;
          const result = await res.json();
          if (result.allPassed) {
            toast.success('✅ Deploy-time visual verification passed');
          } else {
            const failing = result.findings?.filter((f: any) => f.brokenElements?.length > 0).length ?? 0;
            toast.warn(`⚠️ Verification found issues on ${failing} route(s). Check logs.`);
          }
        } catch {
          // non-blocking
        }
      ...
      const errorMsg = err instanceof Error ? err.message : 'Vercel deployment failed';
      // Truncate for toast display (full error already logged above)
      const toastMsg = errorMsg.length > 200 ? errorMsg.slice(0, 200) + '...' : errorMsg;
