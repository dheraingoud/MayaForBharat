...
import { scrubPrompt, looksLikeLeakedSystemPrompt } from '@/lib/workbench/utils/promptSanitizer'
...
export function BuilderPage({ appId }: BuilderPageProps) {
  ...
  // The pipeline instructions + plan context are sent as REQUEST BODY
  // fields (not embedded in the user-visible chat message text) so the
  // chat UI never shows the leaked "MANDATORY BUILD PIPELINE" string.
  ...
  transportBodyRef.current = () => ({
    ...
    // Hidden from the chat UI; the server-side chat route prepends these
    // to the user message before invoking streamText so the model still
    // sees the pipeline instructions but the user never does.
...
You MUST NOT stop or explain anything until all 4 steps have completed. The first version is not done until build passes, tests pass, dev server starts, and all pages render correctly.`,
  ...
  useEffect(() => {
    ...
    if (hasFileActions && !hasShellOrStartActions && autoRunAttemptsRef.current < MAX_AUTO_RUN_CYCLES) {
      ...
      const autoRunHidden = `[SYSTEM] Files were written but the project was not started. Run the FULL pipeline now:
...
🚀 Auto-starting the project (cycle ${autoRunAttemptsRef.current}/${MAX_AUTO_RUN_CYCLES})`;
      setTimeout(() => {
        if (!streamingState.get()) {
          overridePipelineRef.current = autoRunHidden;
          trackedSendMessage({ text: autoRunMsg });
          setTimeout(() => {
            if (overridePipelineRef.current === autoRunHidden) {
              overridePipelineRef.current = null;
            }
          }, 1500);
        }
      }, 500);
  ...
  useEffect(() => {
    const unsubscribe = workbenchStore.autoFixAlert.subscribe((alertData) => {
      ...
      // Simplified auto-fix: fix → install → build → test → start dev
      // The FULL pipeline always runs — no skipped steps.
      const fixDetailedInstructions = source === 'preview'
...
Analyze the error above and fix ALL related source files.`
...
Analyze the error output and fix ALL affected code.`;

      // The user-visible bubble shows ONLY a clean short status — NEVER the
      // heavy pipeline instructions or `IMPORTANT: Reduce ALL errors ...`
      // boilerplate. Those live in the LLM-bound request body so the model
      // still sees them. The chat UI never displays them.
      const userFacingText = source === 'preview'
        ? `Auto-fix attempt ${attempt}/${maxAttempts} — fixing preview error`
        : `Auto-fix attempt ${attempt}/${maxAttempts} — fixing terminal error`;

      const hiddenPayload = `${fixDetailedInstructions}

After fixing, ALWAYS include the FULL pipeline:
1. \`<boltAction type="shell">npm install</boltAction>\`
2. \`<boltAction type="shell">npm run build</boltAction>\`
3. \`<boltAction type="shell">npx vitest run --reporter=verbose 2>&1 || true</boltAction>\`
4. \`<boltAction type="start">npm run dev</boltAction>\`
...
${userFacingText}`;
      ...
      autoFixDebounceRef.current = setTimeout(() => {
        if (streamingState.get()) {
          ...
          pendingAutoFixHiddenRef.current = hiddenPayload;
          return;
        }
        pendingAutoFixRef.current = null;
        pendingAutoFixHiddenRef.current = null;
        overridePipelineRef.current = hiddenPayload;
        trackedSendMessage({ text: messageText });
        setTimeout(() => {
          if (overridePipelineRef.current === hiddenPayload) {
            overridePipelineRef.current = null;
          }
        }, 1500);
      }, 800)
  ...
  useEffect(() => {
    ...
    previewHealthRef.current = setTimeout(() => {
      ...
      const diagHidden = `[SYSTEM] Auto-diagnostics: The dev server was started but no preview appeared after 30 seconds.
...
🔍 Auto-diagnostics: checking why no preview appeared`;

      overridePipelineRef.current = diagHidden;
      trackedSendMessage({ text: diagMsg });
      setTimeout(() => {
        if (overridePipelineRef.current === diagHidden) {
          overridePipelineRef.current = null;
        }
      }, 1500);
    }, 30000)
  ...
  useEffect(() => {
    ...
    let prompt = urlParams.get('prompt') || searchParams?.get('prompt')
    if (!prompt || promptHandledRef.current) return
    promptHandledRef.current = true

    // Scrub anything in the prompt that looks like internal pipeline / system
    // instructions — protect both the chat bubble UI and the model's view.
    if (looksLikeLeakedSystemPrompt(prompt)) {
      const scrub = scrubPrompt(prompt)
      if (scrub.strippedAny) {
        logger.info(`[PromptSanitize] Scrubbed ${scrub.strippedCount} block(s) from incoming prompt`)
        prompt = scrub.text || prompt.split(/---/)[0].trim() || prompt
      }
    }
...
IMPORTANT: Build the app according to this plan. The user has already approved this architecture.`
    ...
    // The user-visible text is just the user's prompt. The MANDATORY BUILD
    // PIPELINE instructions are sent via transport body (transportBodyRef) so
    // the model still sees them on the server side but the chat UI never
    // displays them. We also scrub any blocks that *look like* an internal
    // directive from the prompt itself (defense in depth — some users paste
    // templates).
    const cleanedPrompt = looksLikeLeakedSystemPrompt(prompt)
      ? (scrubPrompt(prompt).text || prompt.split(/---/)[0].trim() || prompt)
      : prompt
    const apiUserText = planContext
      ? `[Model: ${resolvedModel}]
