...
import { Dialog, DialogButton, DialogDescription, DialogRoot, DialogTitle } from '@/lib/workbench/components/ui/Dialog';
import { ThemeSwitch } from '@/lib/workbench/components/ui/ThemeSwitch';
import { ControlPanel } from '@/lib/workbench/components/@settings/core/ControlPanel';
import { SettingsButton, HelpButton } from '@/lib/workbench/components/ui/SettingsButton';
import { Button } from '@/lib/workbench/components/ui/Button';
import { db, getDbPromise, deleteById, getAll, chatId, type ChatHistoryItem, useChatHistory } from '@/lib/workbench/persistence';
import { cubicEasingFn } from '@/lib/workbench/utils/easings';
import { HistoryItem } from './HistoryItem';
import { binDates } from './date-binning';
import { useSearchFilter } from '@/lib/workbench/hooks/useSearchFilter';
...
export const Menu = () => {
    ...
    getDbPromise().then((resolvedDb) => {
      if (resolvedDb) {
        getAll(resolvedDb)
          ...
          .then(setList)
          .catch((error) => toast.error(error instanceof Error ? error.message : 'Failed to load chats'));
      }
    });
  }, []);
  ...
  const deleteChat = useCallback(
    async (id: string): Promise<void> => {
      ...
      await deleteById(resolvedDb, id);
