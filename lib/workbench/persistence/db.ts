...
export async function getNextId(db: IDBDatabase): Promise<string> {
  // Generate a unique mixed alphanumeric ID (e.g., "maya_7kT4X9qR")
  ...
  const candidate = `maya_${id}`;

  // Verify uniqueness (extremely unlikely to collide, but be safe)
  return new Promise((resolve, reject) => {
    const transaction = db.transaction('chats', 'readonly');
    const store = transaction.objectStore('chats');
    ...
    request.onsuccess = () => {
      if (request.result) {
        // Collision (astronomically unlikely) — recurse
        resolve(getNextId(db));
      } else {
        resolve(candidate);
      }
    };
...
export async function forkChat(db: IDBDatabase, chatId: string, messageId: string): Promise<string> {
  ...
  const messages = chat.messages.slice(0, messageIndex + 1) as UIMessage[];
