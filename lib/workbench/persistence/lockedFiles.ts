...
// Track whether migration has been completed to avoid repeated migration attempts
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      if (lockedItemsJson) {
        ...
        if (Array.isArray(lockedItems)) {
          ...
          const updatedItems = lockedItems.map((item: any) => {
 ...
 * Debounced to prevent rapid cascading clears.
 */
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === LOCKED_FILES_KEY) {
      // Debounce cache invalidation to prevent rapid cascading clears
      if (storageSyncTimer) {
