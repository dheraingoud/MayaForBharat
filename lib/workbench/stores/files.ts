...
import { bufferWatchEvents } from '@/lib/workbench/utils/buffer';
import { WORK_DIR } from '@/lib/workbench/utils/constants';
import { computeFileModifications } from '@/lib/workbench/utils/diff';
...
import {
  ...
  migrateLegacyLocks,
} from '@/lib/workbench/persistence/lockedFiles';
import { getCurrentChatId } from '@/lib/workbench/utils/fileLocks';
...
export class FilesStore {
  ...
  /**
   * Interval ID for the periodic lock refresh — stored so it could be cleared in a future destructor.
   */
  #lockRefreshInterval: ReturnType<typeof setInterval> | undefined;
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    // Locked files are loaded in #init() after webcontainer is ready
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('popstate', checkForChatIdChange);
  ...
  async #init() {
    ...
    // Set up file watcher — guard against internal API not being available
     ...
     * NOTE: This interval is intentionally long (30s) to minimize performance impact.
     * The interval ID is stored but not currently cleared since FilesStore has no
     * explicit destructor. For future: expose a destroy() method to clear it.
     */
