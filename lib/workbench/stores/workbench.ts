...
import type { EditorDocument, ScrollPosition } from '@/lib/workbench/components/editor/codemirror/CodeMirrorEditor';
import { ActionRunner } from '@/lib/workbench/runtime/action-runner';
import type { ActionCallbackData, ArtifactCallbackData } from '@/lib/workbench/runtime/message-parser';
...
import { path } from '@/lib/workbench/utils/path';
import { extractRelativePath } from '@/lib/workbench/utils/diff';
import { description } from '@/lib/workbench/persistence';
import Cookies from 'js-cookie';
import { createSampler } from '@/lib/workbench/utils/sampler';
import { createScopedLogger } from '@/lib/workbench/utils/logger';
import type { ActionAlert, DeployAlert, SupabaseAlert } from '@/lib/workbench/types/actions';
...
export class WorkbenchStore {
  ...
  #replayedMessages = new Set<string>();  // Messages from Convex replay — shell/start actions suppressed
  ...
  setReplayedMessages(messages: string[]) {
    this.#replayedMessages = new Set(messages);
  }
  ...
  addArtifact({ messageId, title, id, type }: ArtifactCallbackData) {
    ...
    // Wire auto-fix callback
    runner.onAutoFix = (errorContext) => {
      if (this.#reloadedMessages.has(messageId)) {
        return;
      }
  ...
  async _runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    if (data.action.type === 'file') {
      ...
      // For shell/start/build actions from REPLAYED messages (Convex fallback):
      // mark as executed but DON'T actually run them. The auto-start effect
      // handles npm install + dev server separately.
      // NOTE: Uses #replayedMessages (not #reloadedMessages) so snapshot restore
      // synthetic messages can still execute their shell/start actions normally.
      if (this.#replayedMessages.has(data.messageId)) {
        logger.info(`[_runAction] Skipping replayed ${data.action.type} action — handled by auto-start`);
  ...
  async pushToRepository(
    ...
    try {
              ...
              Object.entries(files).map(async ([filePath, dirent]) => {
                if (dirent?.type === 'file' && dirent.content) {
                  const { data: blob } = await octokit.git.createBlob({
                    ...
                    repo: repo.name,
                    content: btoa(unescape(encodeURIComponent(dirent.content))),
                    encoding: 'base64',
                  });
      ...
      if (isGitLab) {
        const { GitLabApiService: gitLabApiServiceClass } = await import('@/lib/workbench/services/gitlabApiService');
