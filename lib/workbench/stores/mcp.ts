...
import type { MCPConfig, MCPServerTools } from '@/lib/workbench/services/mcpService';
...
export const useMCPStore = create<Store & Actions>((set, get) => ({
  ...
  checkServersAvailabilities: async () => {
    const response = await fetch('/api/workbench/mcp-check', {
      method: 'GET',
    });
...
async function updateServerConfig(config: MCPConfig) {
  const response = await fetch('/api/workbench/mcp-update-config', {
    method: 'POST',
