...
import type { SupabaseUser, SupabaseStats, SupabaseApiKey, SupabaseCredentials } from '@/lib/workbench/types/supabase';
...
export async function fetchSupabaseStats(token: string) {
  ...
  try {
    ...
    const response = await fetch('/api/workbench/supabase', {
      method: 'POST',
      headers: {
...
export async function fetchProjectApiKeys(projectId: string, token: string) {
  ...
  try {
    const response = await fetch('/api/workbench/supabase/variables', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
